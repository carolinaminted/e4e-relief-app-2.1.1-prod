...
const statusStyles: Record<Ticket['status'], string> = {
    ...
    'In Progress': 'bg-[#ff8400]/50 text-[#ffc88a]',
      ...
      <div className="space-y-4">
        ...
        <div className="bg-[#003a70]/50 rounded-lg border border-[#005ca0]">
            <button type="button" onClick={() => toggleSection('submit')} className="w-full flex justify-between items-center text-left p-4" aria-expanded={openSection === 'submit'}>
                <h2 className="text-xl font-semibold text-transparent bg-clip-text bg-gradient-to-r from-[#ff8400] to-[#edda26]">Submit a New Ticket</h2>
                <ChevronIcon isOpen={openSection === 'submit'} />
            </button>
            <div className={`transition-all duration-500 ease-in-out overflow-hidden ${openSection === 'submit' ? 'max-h-[1000px] opacity-100 p-4 pt-0 border-t border-[#005ca0]/50' : 'max-h-0 opacity-0'}`}>
                <form onSubmit={handleSubmit} noValidate className="pt-4 space-y-4">
                    ...
                    <div className="flex justify-end">
                        <button type="submit" disabled={isSubmitting} className="bg-[#ff8400] hover:bg-[#e67700] text-white font-bold py-2 px-6 rounded-md transition-colors duration-200 disabled:bg-gray-500 disabled:cursor-wait">
        ...
        <div className="bg-[#003a70]/50 rounded-lg border border-[#005ca0]">
            <button type="button" onClick={() => toggleSection('view')} className="w-full flex justify-between items-center text-left p-4" aria-expanded={openSection === 'view'}>
                <h2 className="text-xl font-semibold text-transparent bg-clip-text bg-gradient-to-r from-[#ff8400] to-[#edda26]">My Tickets</h2>
                <ChevronIcon isOpen={openSection === 'view'} />
            </button>
            <div className={`transition-all duration-500 ease-in-out overflow-hidden ${openSection === 'view' ? 'max-h-[3000px] opacity-100 p-4 pt-0 border-t border-[#005ca0]/50' : 'max-h-0 opacity-0'}`}>
                <div className="pt-4 space-y-3">
                    {tickets.length > 0 ? (
                        tickets.map(ticket => (
                            <div key={ticket.id} className="bg-[#004b8d]/60 p-4 rounded-md">
