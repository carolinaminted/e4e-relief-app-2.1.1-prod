...
const ProxyPage: React.FC<ProxyApplyPageProps> = ({ navigate, onSubmit, proxyApplications, userProfile, onAddIdentity, mainRef, activeFund }) => {
    ...
    return (
        <div className="p-4 md:p-8 max-w-4xl mx-auto w-full">
            <div className="relative flex justify-center items-center mb-8">
                <button onClick={() => navigate('fundPortal')} className="absolute left-0 md:left-auto md:right-full md:mr-8 text-[var(--theme-accent)] hover:opacity-80 transition-opacity" aria-label="Back to Fund Portal">
                 ...
                 <div className="text-center">
                    <h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[var(--theme-gradient-start)] to-[var(--theme-gradient-end)]">Proxy Application</h1>
                    {userProfile && (
            ...
            {step === 1 && (
                <section className="border border-[var(--theme-border)] bg-[var(--theme-bg-primary)]/30 rounded-lg pb-4 mb-8">
                    <button type="button" onClick={() => setIsHistoryOpen(p => !p)} className="w-full flex justify-between items-center text-left p-4" aria-expanded={isHistoryOpen}>
                        <h2 className="text-xl font-semibold text-transparent bg-clip-text bg-gradient-to-r from-[var(--theme-gradient-start)] to-[var(--theme-gradient-end)]">My Proxy Submissions</h2>
                        ...
                        {proxyApplications.length > 0 ? (
                            ...
                            <button key={app.id} onClick={() => setSelectedApplication(app)} className="w-full text-left bg-[var(--theme-bg-secondary)] p-4 rounded-md flex justify-between items-center hover:bg-[var(--theme-border)]/50 transition-colors duration-200">
                                ...
                                <div className="text-right">
                                    <p className="font-bold text-md text-transparent bg-clip-text bg-gradient-to-r from-[var(--theme-gradient-start)] to-[var(--theme-gradient-end)]">${app.requestedAmount.toFixed(2)}</p>
