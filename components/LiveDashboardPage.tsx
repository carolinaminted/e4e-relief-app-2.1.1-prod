...
const MetricCard: React.FC<{ title: string; children: React.ReactNode; className?: string }> = ({ title, children, className = '' }) => (
    <div className={`bg-[var(--theme-bg-secondary)] p-6 rounded-lg border border-[var(--theme-border)] flex flex-col ${className}`}>
    ...
    const TabButton: React.FC<{ tabName: 'home' | 'payments'; label: string }> = ({ tabName, label }) => (
        <button
            ...
            className={`px-4 py-2 text-sm font-semibold rounded-t-md transition-colors duration-200 border-b-2 ${
                activeTab === tabName
                    ? 'text-white border-[var(--theme-accent)] bg-[var(--theme-bg-secondary)]'
            ...
            {activeTab === 'home' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-[fadeIn_0.5s_ease-out]">
                    <MetricCard title="Total Grant Payments (USD)">
                        <p className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[var(--theme-gradient-start)] to-[var(--theme-gradient-end)]">${stats.totalAwarded.toLocaleString()}</p>
                    </MetricCard>
                    ...
                    <MetricCard title="Recently Registered Users">
                        <div className="space-y-2 w-full">
                            {stats.recentUsersData.map((user, index) => (
                                <div key={index} className="grid grid-cols-10 gap-2 text-sm p-2 rounded hover:bg-[var(--theme-bg-primary)]/50">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-[fadeIn_0.5s_ease-out]">
                    <MetricCard title="Total Payments Disbursed (USD)">
                     ...
                     <MetricCard title="Average Payment Amount (USD)">
                        <p className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[var(--theme-gradient-start)] to-[var(--theme-gradient-end)]">${stats.avgPayment.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                    </MetricCard>
                     <MetricCard title="Payments in Last 30 Days">
                        <p className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[var(--theme-gradient-start)] to-[var(--theme-gradient-end)]">{stats.paymentsLast30Days.toLocaleString()}</p>
                    </MetricCard>
                            ...
                            <div className="mb-4 px-1">
                                <div className="relative">
                                    ...
                                    <input
                                        type="search"
                                        id="payments-search"
                                        className="block w-full p-3 pl-10 text-sm text-white bg-[var(--theme-bg-primary)] border border-[var(--theme-border)] rounded-lg focus:ring-[var(--theme-accent)] focus:border-[var(--theme-accent)]"
                                ...
                                <table className="min-w-full text-sm text-left">
                                    ...
                                    <tbody className="divide-y divide-[var(--theme-border)]/50">
                                        {processedRecentPayments.length > 0 ? (
                                            ...
                                            <tr key={row.id} className="hover:bg-[var(--theme-bg-primary)]/50">
