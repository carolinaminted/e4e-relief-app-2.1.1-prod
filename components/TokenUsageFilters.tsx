...
const FilterSelect: React.FC<{ label: string, value: string, onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void, options: string[] }> = ({ label, value, onChange, options }) => (
    <div className="flex-1 min-w-[150px]">
        ...
        <select value={value} onChange={onChange} className="w-full bg-[var(--theme-bg-primary)] border border-[var(--theme-border)] rounded-md p-2 text-white focus:ring-2 focus:ring-[var(--theme-accent)] focus:border-[var(--theme-accent)]">
    ...
    return (
        <div className="flex-1 min-w-[150px]">
            ...
            <input 
                ...
                value={value} 
                onChange={onChange} 
                className="w-full bg-[var(--theme-bg-primary)] border border-[var(--theme-border)] rounded-md p-2 text-white focus:ring-2 focus:ring-[var(--theme-accent)] focus:border-[var(--theme-accent)]" 
            />
        </div>
    );
};
...
export const TokenUsageFilterModal: React.FC<TokenUsageFilterModalProps> = ({ 
  ...
  return createPortal(
    ...
    >
      <div 
        className="bg-[var(--theme-bg-secondary)] rounded-lg shadow-xl p-6 w-full max-w-3xl m-4 relative border border-[var(--theme-border)]"
